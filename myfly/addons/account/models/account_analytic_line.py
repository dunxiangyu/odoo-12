...
class AccountAnalyticLine(models.Model):
    ...
    def _on_change_unit_amount(self):
        if not self.product_id:
            return {}

        result = 0.0
        prod_accounts = self.product_id.product_tmpl_id.get_product_accounts()
    ...
    def view_header_get(self, view_id=None, view_type='form'):
        ...
        if context.get('account_id', False):
            analytic_account = self.env['account.analytic.account'].search([('id', '=', context['account_id'])],
                                                                           limit=1)
